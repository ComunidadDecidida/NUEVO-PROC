        ...
        foreach ($factura in $facturas) {
            try {
                ...
                if ($tieneCicloEscolar) {
                    ...
                    $nuevaVigencia = Get-NextVigenciaDate -VigenciaDia $VigenciaDia -Observaciones $observaciones
                    $tipoVigencia = "normal"
                    
                    # Registrar fechas detectadas para vigencias normales
                    $anioActual = (Get-Date).Year
                    $fechasEnObservaciones = Extract-FechasFromObservaciones -Observaciones $observaciones -AnioActual $anioActual
                    if ($fechasEnObservaciones.Count -gt 0) {
                        $fechasDetectadas += @{
                            clienteId = $clienteId
                            observaciones = $observaciones
                            fechasEncontradas = $fechasEnObservaciones
                            vigenciaCalculada = $nuevaVigencia.ToString("yyyy-MM-dd")
...
function Get-NextVigenciaDate {
    ...
    # Extraer meses y años de las observaciones
    $fechasEncontradas = Extract-FechasFromObservaciones -Observaciones $Observaciones -AnioActual $anioActual
    
    if ($fechasEncontradas.Count -eq 0) {
        # Si no se encuentran fechas válidas, usar lógica anterior como fallback
        $hoy = Get-Date
        $anio = $hoy.Year
        $mes = $hoy.Month
    ...
    # Encontrar el mes mayor de todas las fechas válidas
    $fechaMayor = $fechasEncontradas | Sort-Object { $_.Year }, { $_.Month } | Select-Object -Last 1
    
    # Calcular el mes siguiente al mes mayor
    $anioVigencia = $fechaMayor.Year
    $mesVigencia = $fechaMayor.Month + 1
    ...
    # Ajustar día de vigencia si el mes no tiene suficientes días
    $diasEnMes = [DateTime]::DaysInMonth($anioVigencia, $mesVigencia)
    $diaVigencia = [Math]::Min($VigenciaDia, $diasEnMes)
    
    $proximaVigencia = Get-Date -Year $anioVigencia -Month $mesVigencia -Day $diaVigencia -Hour 0 -Minute 0 -Second 0
    
    return $proximaVigencia
}

# Función para extraer fechas de las observaciones
function Extract-FechasFromObservaciones {
    param(
        [string]$Observaciones,
        [int]$AnioActual
    )
    ...
    # Patrones de búsqueda para diferentes formatos de fecha
    $patrones = @(
        # Patrón: "junio a noviembre 2025", "diciembre a marzo de 2026"
        '([A-ZÁÉÍÓÚ]+)\s+A\s+([A-ZÁÉÍÓÚ]+)\s+(?:DE\s+)?(\d{4})',
        # Patrón: "junio-noviembre 2025", "diciembre-marzo 2026"
        '([A-ZÁÉÍÓÚ]+)\s*-\s*([A-ZÁÉÍÓÚ]+)\s+(\d{4})',
        # Patrón: "junio/noviembre 2025"
        '([A-ZÁÉÍÓÚ]+)\s*/\s*([A-ZÁÉÍÓÚ]+)\s+(\d{4})',
        # Patrón: "mes año" (ej: "junio 2025")
        '([A-ZÁÉÍÓÚ]+)\s+(\d{4})',
        # Patrón: "año mes" (ej: "2025 junio")
        '(\d{4})\s+([A-ZÁÉÍÓÚ]+)'
    )
    
    $observacionesUpper = $Observaciones.ToUpper()
    
    foreach ($patron in $patrones) {
        $matches = [regex]::Matches($observacionesUpper, $patron)
        
        foreach ($match in $matches) {
            try {
                if ($patron -match 'A\s+' -or $patron -match '-' -or $patron -match '/') {
                    # Patrón de rango (mes1 a mes2 año)
                    $mes1Texto = $match.Groups[1].Value.Trim()
                    $mes2Texto = $match.Groups[2].Value.Trim()
                    $anio = [int]$match.Groups[3].Value
                    
                    # Validar año
                    if ($anio -lt $AnioActual) {
                        continue
                    }
                    ...
                    foreach ($mesKey in $mesesEspanol.Keys) {
                        if ($mes1Texto.StartsWith($mesKey)) {
                            $mes1 = $mesesEspanol[$mesKey]
                        }
                        if ($mes2Texto.StartsWith($mesKey)) {
                            $mes2 = $mesesEspanol[$mesKey]
                        }
                    }
                    
                    if ($mes1 -and $mes2) {
                        # Agregar todos los meses en el rango
                        $mesActual = $mes1
                        $anioActualRango = $anio
                        
                        while ($true) {
                            $fechasValidas += @{
                                Month = $mesActual
                                Year = $anioActualRango
                                Texto = "$($mes1Texto)-$($mes2Texto) $anio"
                            }
                            
                            if ($mesActual -eq $mes2 -and $anioActualRango -eq $anio) {
                                break
                            }
                            ...
                            if ($anioActualRango -gt $anio + 2) {
                                break
                            }
                        }
                    }
                    
                } elseif ($patron -match '\(\d\{4\}\).*\(\[A-Z') {
                    # Patrón: "año mes"
                    $anio = [int]$match.Groups[1].Value
                    $mesTexto = $match.Groups[2].Value.Trim()
                    ...
                    foreach ($mesKey in $mesesEspanol.Keys) {
                        if ($mesTexto.StartsWith($mesKey)) {
                            $fechasValidas += @{
                                Month = $mesesEspanol[$mesKey]
                                Year = $anio
                                Texto = "$mesTexto $anio"
                            }
                            break
                    ...
                    # Patrón: "mes año"
                    $mesTexto = $match.Groups[1].Value.Trim()
                    $anio = [int]$match.Groups[2].Value
